...
class Resellers_API_Client {
     ...
     */
    public function __construct( Resellers_API_Plugin $plugin ) {
        $this->plugin = $plugin;
    }
    ...
    public function request( $method, $endpoint, array $args = array() ) {
        ...
        if ( empty( $api_key ) ) {
            return new WP_Error( 'resellers_api_missing_key', __( 'Lütfen önce API anahtarınızı kaydedin.', 'resellers-api' ) );
        }
        ...
        if ( null === $data && JSON_ERROR_NONE !== json_last_error() ) {
            return new WP_Error( 'resellers_api_invalid_response', __( 'API yanıtı çözümlenemedi.', 'resellers-api' ) );
        }
        ...
        $message = isset( $data['message'] ) ? $data['message'] : __( 'Bilinmeyen API hatası oluştu.', 'resellers-api' );

        return new WP_Error( 'resellers_api_api_error', $message, array( 'status' => $code ) );
    }
}
