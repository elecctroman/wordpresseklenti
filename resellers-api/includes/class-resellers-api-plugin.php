...
require_once RESELLERS_API_PATH . 'includes/class-resellers-api-client.php';
require_once RESELLERS_API_PATH . 'includes/class-resellers-api-admin.php';

class Resellers_API_Plugin {

    const OPTION_KEY         = 'resellers_api_options';
    const LICENSE_OPTION     = 'resellers_api_license_host';
    const PRODUCTS_OPTION    = 'resellers_api_last_products';
    const NOTICE_TRANSIENT   = 'resellers_api_notice_buffer';
    const BALANCE_TRANSIENT  = 'resellers_api_balance_cache';
    const USER_API_KEY_META  = '_resellers_api_key';
    ...
    protected function __construct() {
        $this->plugin_basename = plugin_basename( RESELLERS_API_FILE );
        $this->api_client      = new Resellers_API_Client( $this );
        $this->admin           = new Resellers_API_Admin( $this, $this->api_client );

        register_activation_hook( RESELLERS_API_FILE, array( $this, 'activate' ) );
    ...
    public function register_settings() {
        register_setting( 'resellers_api_settings', self::OPTION_KEY, array( $this, 'sanitize_options' ) );
    }
                ...
                function() use ( $stored_host, $current_host ) {
                    printf(
                        ...
                        esc_html( sprintf(
                            ...
                            __( 'Resellers API eklentisi bu alan adına lisanslı: %1$s. Mevcut alan adı (%2$s) ile eşleşmediği için eklenti devre dışı bırakıldı.', 'resellers-api' ),
                            $stored_host,
                            $current_host
                        ) )
                    );
    ...
    protected function get_license_option_name() {
        ...
        return 'resellers_api_license_host';
    }
    ...
    public function maybe_load_textdomain() {
        load_plugin_textdomain( 'resellers-api', false, dirname( $this->plugin_basename ) . '/languages' );
    }
    ...
    public function maybe_render_docs_template( $template ) {
        ...
        $docs_template = RESELLERS_API_PATH . 'templates/api-docs.php';
    ...
    public function register_admin_bar_nodes( $admin_bar ) {
        ...
        $balance_text = __( 'Resellers API Bakiye', 'resellers-api' );

        if ( null !== $balance_data && isset( $balance_data['credit'] ) ) {
            $balance_text = sprintf(
                __( 'Resellers API Bakiye: %s', 'resellers-api' ),
        ...
        $admin_bar->add_menu(
            array(
                'id'    => 'resellers-api-balance',
                'title' => esc_html( $balance_text ),
                'href'  => admin_url( 'admin.php?page=resellers-api' ),
                'meta'  => array( 'title' => __( 'Resellers API Ayarları', 'resellers-api' ) ),
            )
        );
        ...
        if ( ! empty( $notices ) && is_array( $notices ) ) {
            $admin_bar->add_menu(
                array(
                    'id'     => 'resellers-api-alerts',
                    'parent' => 'top-secondary',
                    'title'  => __( 'Resellers API Bildirimleri', 'resellers-api' ),
                    'href'   => admin_url( 'admin.php?page=resellers-api' ),
                )
            );
        }
    ...
    public function maybe_handle_account_actions() {
        if ( empty( $_POST['resellers_api_action'] ) ) {
            return;
        }
        ...
        $action = sanitize_text_field( wp_unslash( $_POST['resellers_api_action'] ) );

        if ( 'regenerate_api_key' !== $action ) {
            return;
        }

        $nonce = isset( $_POST['_resellers_api_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_resellers_api_nonce'] ) ) : '';

        if ( ! wp_verify_nonce( $nonce, 'resellers_api_regenerate_api_key' ) ) {
            if ( function_exists( 'wc_add_notice' ) ) {
                wc_add_notice( __( 'Güvenlik doğrulaması başarısız oldu.', 'resellers-api' ), 'error' );
            }
        ...
        if ( function_exists( 'wc_add_notice' ) ) {
            wc_add_notice( __( 'API anahtarınız yenilendi.', 'resellers-api' ), 'success' );
        }
    ...
    public function render_customer_api_panel() {
        ...
        ?>
        <section class="resellers-api-customer-api">
            <h2><?php esc_html_e( 'Reseller API Bilgileriniz', 'resellers-api' ); ?></h2>
            <p><?php esc_html_e( 'Bu anahtar ile ürünleri sitenize aktarabilir ve siparişleri otomatikleştirebilirsiniz. Anahtarınızı güvende tutunuz.', 'resellers-api' ); ?></p>
            <table class="shop_table shop_table_responsive">
                <tbody>
                    <tr>
                        <th><?php esc_html_e( 'API Temel Adresi', 'resellers-api' ); ?></th>
                        <td><code><?php echo esc_html( $api_base ); ?></code></td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e( 'API Anahtarı', 'resellers-api' ); ?></th>
                        <td><code><?php echo esc_html( $api_key ); ?></code></td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e( 'Dokümantasyon', 'resellers-api' ); ?></th>
                        <td><a href="<?php echo esc_url( $docs_url ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Reseller API Dokümantasyonu', 'resellers-api' ); ?></a></td>
                    </tr>
                </tbody>
            </table>
            <form method="post" class="resellers-api-key-regenerate">
                <?php wp_nonce_field( 'resellers_api_regenerate_api_key', '_resellers_api_nonce' ); ?>
                <input type="hidden" name="resellers_api_action" value="regenerate_api_key" />
                <button type="submit" class="button" onclick="return confirm('<?php echo esc_js( __( 'Yeni bir anahtar oluşturmak üzeresiniz. Eski anahtar geçersiz hale gelecektir. Devam edilsin mi?', 'resellers-api' ) ); ?>');">
                    <?php esc_html_e( 'API Anahtarını Yenile', 'resellers-api' ); ?>
                </button>
            </form>
        </section>
        <?php
    ...
    public function authenticate_rest_request( $request ) {
        ...
        if ( empty( $api_key ) ) {
            return new WP_Error( 'resellers_api_missing_key', __( 'API anahtarı gerekli.', 'resellers-api' ), array( 'status' => 401 ) );
        }

        $user_id = $this->find_user_by_api_key( $api_key );

        if ( ! $user_id ) {
            return new WP_Error( 'resellers_api_invalid_key', __( 'API anahtarınız doğrulanamadı.', 'resellers-api' ), array( 'status' => 401 ) );
        }

        if ( $request instanceof WP_REST_Request ) {
            $request->set_attribute( 'resellers_api_user_id', $user_id );
        }
    ...
    public function rest_get_products( $request ) {
        if ( ! class_exists( 'WooCommerce' ) ) {
            return new WP_Error( 'resellers_api_woocommerce_missing', __( 'WooCommerce etkin değil.', 'resellers-api' ), array( 'status' => 503 ) );
        }
    ...
    public function rest_get_single_product( $request ) {
        ...
        if ( ! $product || 'publish' !== $product->get_status() ) {
            return new WP_Error( 'resellers_api_product_not_found', __( 'Ürün bulunamadı.', 'resellers-api' ), array( 'status' => 404 ) );
        }
    ...
    public function rest_create_order( $request ) {
        ...
        if ( method_exists( $order, 'set_created_via' ) ) {
            $order->set_created_via( 'resellers-api' );
        }
        ...
        try {
            ...
            return new WP_Error( 'resellers_api_order_error', $exception->getMessage(), array( 'status' => 400 ) );
        }

        $customer_id = $request->get_attribute( 'resellers_api_user_id' );
        ...
        $order->update_meta_data( '_resellers_api_consumer', $request->get_attribute( 'resellers_api_user_id' ) );
        $order->save();
